...
func TestSendMessageV3RejectedReasonIsAnError(t *testing.T) {
	// v3 拒收返回的是 200 + reason≠1，不识别就会把消息静默丢掉
	var mu sync.Mutex
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		mu.Lock()
		defer mu.Unlock()
	...
	_, err := ctx.SendMessageWithResult(&MsgSendReq{
		ChannelID:   "g_1",
		ChannelType: common.ChannelTypeGroup.Uint8(),
		Payload:     []byte("tip"),
	})
	if err == nil {
		t.Fatal("reason=13(NotInWhitelist) 应该报错，实际返回成功")
	}
}

func TestSendMessageV3ReadsResultFromTopLevel(t *testing.T) {
	// v3 的 /message/send 把结果放在响应顶层，没有 data 包裹；
	// 按 data 取会让 message_id 恒为 0（机器人接口就是把它原样返回给第三方的）
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"message_id":2091631714844545024,"message_seq":7,"reason":1}`))
	}))
	defer server.Close()
	...
	got, err := ctx.SendMessageWithResult(&MsgSendReq{
		FromUID:     "u_a",
		ChannelID:   "u_b",
		ChannelType: common.ChannelTypePerson.Uint8(),
		Payload:     []byte("hi"),
	...
	if got.MessageID != 2091631714844545024 || got.MessageSeq != 7 {
		t.Fatalf("解析结果 = %+v, want message_id=2091631714844545024 message_seq=7", got)
	}
}

func TestSendMessageV2StillReadsDataWrapper(t *testing.T) {
	// v2 引擎的响应仍然是 {"data":{...}}，不能被 v3 的顶层解析改坏
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"status":200,"data":{"message_id":123,"message_seq":9,"client_msg_no":"cmn-1"}}`))
	}))
	defer server.Close()
	...
	if got.MessageID != 123 || got.MessageSeq != 9 || got.ClientMsgNo != "cmn-1" {
		t.Fatalf("解析结果 = %+v, want v2 的 data 包裹字段", got)
	}
}
